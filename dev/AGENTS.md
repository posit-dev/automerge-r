...
Guidance for AI coding agents working **on** the automerge package.
automerge provides R bindings to the Automerge Conflict-free Replicated
Data Type (CRDT) library via its C FFI, enabling automatic merging of
concurrent changes across distributed systems without conflicts. Zero R
package dependencies (only base R).
...
- R layer provides idiomatic S3 methods (`$`, `[[`, `length`, `names`,
  `as.list`)
- C layer wraps the automerge-c API with memory-safe external pointer
  management
