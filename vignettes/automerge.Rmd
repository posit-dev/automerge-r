...
for (i in seq_along(history)) {
  change <- am_change_from_bytes(history[[i]])
  cat(
    sprintf(
      "Change %d: seq=%g, message=%s\n",
      i,
      am_change_seq(change),
      am_change_message(change) %||% "(none)"
    )
  )
}

# Parse once, extract many fields
change <- am_change_from_bytes(history[[2]])
...
## Standalone Actor IDs

Actor IDs can be created and converted without a document, useful for pre-assigning identities:

```{r}
# Generate a random actor ID
actor <- am_actor_id_random()

# Convert between bytes and hex string
hex <- am_actor_id_to_string(actor)
cat("Actor ID:", hex, "\n")

restored_actor <- am_actor_id_from_string(hex)
identical(actor, restored_actor)  # TRUE

# Use with a new document
doc16 <- am_create(actor)
identical(am_get_actor(doc16), actor)  # TRUE
