...
# Actor ID (standalone)
actor <- am_actor_id_random() # Generate random actor ID
actor <- am_actor_id_from_string("0123...cdef") # From hex string
actor <- am_actor_id_from_bytes(raw_bytes) # From/validate raw bytes
hex <- am_actor_id_to_string(actor) # Convert to hex string
bytes <- am_actor_id_to_bytes(actor) # Validate and return bytes
...
# Change introspection (parse once, extract many fields)
...
bytes <- am_change_to_bytes(change) # Serialize back to raw
```
